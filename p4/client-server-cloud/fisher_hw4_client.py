...
while True:
    ...
    print('5. Communicate with other devices')
    print('6. Communicate with Cloud')
    print('7. QUIT device-ID')
    ...
    if command == '1': #Executes the command to Register the device with the server
        ...
        query_input = input('Please enter 1 or 2: ')
        s = socket(AF_INET, SOCK_STREAM)
        s.connect(server_address)
        ...
        if snd_rec == '1':            #executes if sending data to another device
            ...
            dst_port = int(input("Please enter the destination's Port number: "))
            print('-'*40)
            print('1. Run Ping')
        ...
        if cloud_cnt == 0:
            s = socket(AF_INET, SOCK_STREAM)
            s.connect(server_address)
            client_data = 'CLOUD_START ' + host_name
            oauth_file = open('mycreds2.txt', 'w')
            s.sendto(client_data, server_address)
            oauth_file_towrite=s.recvfrom(1024)
            oauth_file.write(oauth_file_towrite[0])
            oauth_file.close()
            cloud_cnt += 1
            s.close()
        ...
        s = socket(AF_INET, SOCK_STREAM)
        s.connect(server_address)
        result = ping_udp('www.google.com')
        s.sendto('GET_ID', server_address)
        file_id = str(s.recvfrom(1024))
        print file_id[2:-8]
        file1 = drive.CreateFile({'id': file_id[2:-8]})
        result_file = file1.GetContentString()
        result_file = result_file + '\n' + '-'*40 + '\n' + host_name + '\n' + '-'*40 + '\n' + result
        file1.SetContentString(result_file)
        file1.Upload()
        s.close()'''
        ...
        print('1. Run Ping')
        print('2. Run Traceroute')
        cld_input = raw_input('Please select 1 or 2: ')
        addrss = raw_input('Please enter a url or IP address: ')
        if cld_input == '1':     #executes if running a ping command
            print('Getting Cloud File ID')
            s = socket(AF_INET, SOCK_STREAM)
            s.connect(server_address)
            s.sendto('GET_ID', server_address)
            file_id = str(s.recvfrom(1024))
            ...
            file1.Upload()
            s.close()
        elif cld_input == '2':   #executes if running a traceroute command
            print('Getting Cloud File ID')
            s = socket(AF_INET, SOCK_STREAM)
            s.connect(server_address)
            s.sendto('GET_ID', server_address)
            ...
            print(file_id[2:-8])
            file1 = drive.CreateFile({'id': file_id[2:-8]})
            print('Running Traceroute for ' + addrss)
            result = traceroute_udp(addrss)
            result_file = file1.GetContentString()
            result_file = result_file + '\n' + '-'*40 + '\n' + host_name + '\n' + '-'*40 + '\n' + result
            file1.SetContentString(result_file)
        ...
        s = socket(AF_INET, SOCK_STREAM)
        s.connect(server_address)
        s.sendto('RESULT_UPDATE', server_address)
        received_server = s.recvfrom(1024) #receives response from server
        print('-'*40)
        print(received_server[0])           #displays the response from server
        print('-'*40)
